...
public unsafe static partial class JNI
{
    ...
    public static int RegisterNatives(JClass cls, JNINativeMethod[] methods)
    {
        //IntPtr unmanagedAddress = Marshal.StructureToPtr<JNINativeMethod[]>(methods,, false);
        //jint RegisterNatives(JNIEnv* env, jclass clazz, const JNINativeMethod* methods, jint nMethods);
        int ret = Env->Functions->RegisterNatives(Env, cls.Handle, methods, methods.Length);
        //Marshal.FreeHGlobal(unmanagedAddress);
        if (ret != 0) throw new Exception();
        return ret;
    }

    public static int UnregisterNatives(JClass cls)
    {
        int ret = Env->Functions->UnregisterNatives(Env, cls.Handle);
        return ret;
    }
